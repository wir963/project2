...
#define APP_MODULE_NAME "GUSEARCH"
  ...
  if (module == APP_MODULE_NAME || module == "APP")
    {
      ...
      Ptr<GUSearch> application = m_nodeContainer.Get(nodeNumber)->GetApplication(0)->GetObject<GUSearch> ();
      ...
      if (command == "VERBOSITY" || command == "VERBOSE")
        ...
        {
          Simulator::Schedule (MilliSeconds (time.GetMilliSeconds()), &GUSearch::ProcessCommand, application, tokens);
          return;
        }
    }
...
main (int argc, char *argv[])
{
  ...
  NS_LOG_INFO ("Installing GUSearch Application...");

  GUSearchHelper appHelper = GUSearchHelper ();
  ApplicationContainer apps = appHelper.Install (realNodeContainer);
  for (uint32_t i = 0 ; i < totalNodes ; i++)
    {
      Ptr<GUSearch> application = realNodeContainer.Get(i)->GetApplication(0)->GetObject<GUSearch> ();
