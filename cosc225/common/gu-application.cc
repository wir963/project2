...
void
GUApplication::SetRealStack (bool realStack)
{
  m_realStack = realStack;
}

bool
GUApplication::IsRealStack ()
{
  return m_realStack;
}

void
GUApplication::SetLocalAddress(Ipv4Address local)
{
  m_local = local;
}

Ipv4Address
GUApplication::GetLocalAddress()
{
  return m_local;
}
...
void
GUApplication::SetNodeAddressMap (std::map<uint32_t, Ipv4Address> nodeAddressMap)
{
  m_nodeAddressMap = nodeAddressMap;
}

void
GUApplication::SetAddressNodeMap (std::map<Ipv4Address, uint32_t> addressNodeMap)
{
  m_addressNodeMap = addressNodeMap;
}

Ipv4Address
GUApplication::ResolveNodeIpAddress (std::string nodeId)
{
  if (!IsRealStack())
  {  
    uint32_t nodeNumber;
...
std::string
GUApplication::ReverseLookup (Ipv4Address ipAddress)
{
  if (!IsRealStack())
  {
